...
class EasyGoogleMap {
	...
	function InitJs() {
		...
		if ($is_error) { 
			...
			for ($i=$cnt_add-1; $i>=0; $i--) {
				...
				$ret .= "	var point = new GLatLng({$this->latitud},{$this->longitud});\n";
	...
	function CheckConf() {
        ...
        function GeoCoder($address){
            $calle = strtok($address, ' ');
            $numero = substr($address, -4);
            $geocoderResponse = new HTTP_Request("http://maps.google.com/maps/geo?q=".$numero."+".$calle."+montevideo&output=xml&key={$this->mMapKey}");
            $geocoderResponse->sendRequest();
            $r = $geocoderResponse->getResponseBody();
            
            $xml = new SimpleXMLElement($r);
            $coordenadas = $xml->Response->Placemark->Point->coordinates;
            $this->latitud = strtok($coordenadas, ',');
            $lenLat = strlen($this->latitud);
            $auxCadena = substr_replace($coordenadas, '',0, $lenLat+1);
            $this->longitud = strtok($auxCadena,',');
            echo $coordenadas;
        }
