...
class pasajeroActions extends sfActions
{
  ...
  public function executeNew(sfWebRequest $request)
  {
    ...
    if(!empty($idOrden)){
        ...
        $pasajero->setOrden($objOrden->getId());
        $this->form = new PasajeroForm($pasajero);
        $this->form->setIdOrden($idOrden);
            $this->form->setDefault('hora', array(
          'year' => date('Y'),
          'month' => date('m'),
          'day' => date('d')));
...
//));
  ...
  public function executeMap(sfWebRequest $request){
      ...
      foreach($gm2->GetPlacemark() as $placemark){
            ...
            $this->latitud = $placemark->getLatitude();
            $this->longitud = $placemark->getLongitude();
      ...
      // END OF ACTION
      $this->idCliente = $request->getParameter('orden');
      if($dirValida){
          $this->generated_js = $this->gMap->getJavascript();
          $this->anterior = $request->getReferer();
      }else{
          $this->redirect('error/index?orden='.$request->getParameter('orden'));
      }
